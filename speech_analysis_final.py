...
# ── LLM scoring for open-ended exercises ────────────────────────────────
...
def generate_clarity_report(audio_path):
    ...
    report = "Your pronunciation clarity:\n"
...
def score_pronunciation_phonetic(audio_path):
    """Same idea as score_pronunciation, but using the phoneme-specialized model."""
    model, processor = _get_phoneme_model()
    speech, sr = librosa.load(audio_path, sr=16000)
    input_values = processor(speech, sampling_rate=16000, return_tensors="pt").input_values
...
def process_exercise_attempt(audio_path, exercise_id, subcategory, user_id, session_id,
                              scoring_type=None, expected_answer=None,
                              exercise_instructions=None, exercise_title=None):
    analysis = analyze_speech(audio_path)
    weak_phonemes = identify_weak_phonemes(analysis["audio_path_used"], analysis["segments"])
